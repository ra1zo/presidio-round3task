...
include('includes/config.php');
	...
	if(isset($_GET['accept'])){
		$bid=$_GET['bid'];
		$sql = 'UPDATE tblcarwashbooking SET status="Old" where bookingId=:bid';
		$query = $dbh->prepare($sql);
		$query->bindParam('bid', $_GET['bid'], PDO::PARAM_INT);
		$query->execute();
	}
	if(isset($_GET['decline'])){
		$id=$_GET['bid'];
		$sql = 'UPDATE tblcarwashbooking SET status="decline" where bookingId=:bid';
		$query = $dbh->prepare($sql);
		$query->bindParam('bid', $_GET['bid'], PDO::PARAM_INT);
		$query->execute();
	}
         ...
         <table id="table">
            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td>
                     <a href="?bid=<?php echo htmlentities($result->bookingId);?>&accept=1">Accept</a>
                     <a href="?bid=<?php echo htmlentities($result->bookingId);?>&decline=1">Decline</a>
                  </td>
                  <?php } ?>
               </tr>
               <?php } else { ?>
